...
if [ -z "${SUDO_USER:-}" ] || [ "$SUDO_USER" = "root" ]; then
    echo "ClaudeBar postinstall: SUDO_USER not set; skipping statusline configuration."
    echo "  Run \`claudebar --configure-statusline\` manually after first launch."
    exit 0
fi
...
if [ -z "$USER_HOME" ] || [ ! -d "$USER_HOME" ]; then
    echo "ClaudeBar postinstall: could not resolve home dir for $TARGET_USER; skipping."
    exit 0
fi
...
if [ -z "$PYTHON" ]; then
    echo "ClaudeBar postinstall: python not found; skipping statusline configuration."
    exit 0
fi
...
if os.path.exists(path):
    try:
        ...
        if not isinstance(settings, dict):
            print(f"ClaudeBar: {path} is not a JSON object; skipping.", file=sys.stderr)
            sys.exit(0)
    except json.JSONDecodeError as e:
        print(f"ClaudeBar: {path} is not valid JSON ({e}); skipping.", file=sys.stderr)
        sys.exit(0)
...
try:
    ...
    print(f"ClaudeBar: statusline {mode} complete ({path})")
except Exception:
    try:
        os.unlink(tmp_path)
    except OSError:
